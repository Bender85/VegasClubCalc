    ...
    var helpers = {
      ...
      */
      ...
      updateQty: function updateQty($this) {
        ...
        var qty = parseInt($this.closest('.formCounter').find('.iconCounter').text()) || 0;

        // Updates the qty
        var new_qty = $this.hasClass('counterPlus') ? qty + 1 : qty - 1;

        if (new_qty >= 0) {
          $this.closest('.formCounter').find('.iconCounter').text(new_qty);
      ...
      events: function events() {
        ...
        $('.counterPlus, .counterMinus').click(function () {
      ...
      generateTeamsOptions: function generateTeamsOptions(selectedLeague, callback) {
        ...
        $(leagues[selectedLeague]).each(function (i, data) {
          teamDropdowns += `<option team="${i}" class="calculator-options-teams" value="${i}">${$(this)[0].name}</option>`;
          // teamDropdowns += '<span team="' + i + '" badge="">';
          // teamDropdowns += '' + $(this)[0].name + '</span>';
        });
        // var teamDropdowns = '<option class="calculator-options-teams">';
        //
        // $(leagues[selectedLeague]).each(function (i, data) {
        //   teamDropdowns += '<span team="' + i + '" badge="">';
        //   teamDropdowns += '' + $(this)[0].name + '</span>';
        // });
        //
        // teamDropdowns += '</option>';

        $('.teams__dropdown').attr('team', 'none');

        $('.dinamic-dropdown').html(teamDropdowns);

        // Default placeholder
        // for the teams dropdowns
        // 'Please select..' dropdown
        // $('span').text(that.cachePlaceholderTeam);
      ...
      events: function events() {
        ...
        // Close dropdowns on clicks outside
          ...
          .on('change', 'select', function () {
            // console.log($(this).attr('team'));
            var selectedTeam = $(this).attr('team') || false,
            ...
            // Teams dropdown
            if ($('select option').hasClass('calculator-options-teams')) {
              var option = $('option:selected', this).attr('team');
              console.log(option);
            ...
            if ($(this).closest('.leagues__dropdown').parent('.compare-table-select').length > 0) {
              ...
              var formSelectedLeague = $('.calculator__item-body .leagues__dropdown').attr('league');
              var formSelectedTeam = $('.calculator__item-body .teams__dropdown ').attr('team');
      ...
      events: function events() {
        ...
        $('.footy-calculator__recepit').on('updatePrices', function () {
          // debugger;
          console.log($('select').attr('team') == 'none' ? false : $('select').attr('team'));
          var total = 0,
            // league = $('.leagues__dropdown').attr('league') == '' ? false : $('.leagues__dropdown').attr('league'),
          ...
          if (team) {
            ...
            $('.subtotalList').each(function () {
              ...
              if (!$(this).hasClass('subtotalListItem')) {
                ...
                $(this).find('.subtotalListItem').each(function () {
            ...
            // Calculate total
            $('.total').html(curency + parseFloat(total).toFixed(2));
          }
